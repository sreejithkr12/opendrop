...
class MainMenuWindowView(GtkAppWinComponentView):
...
class MainMenuWindowPresenter(Presenter):
    pass


class _MainMenuWindow(GtkAppWinComponent):
    view = ...  # type: MainMenuWindowView
    presenter = ...  # type: MainMenuWindowPresenter

    def _set_up(self) -> None:
        # Connect to view events.
        self._event_conns = [
            self.view.on_about_btn_clicked.connect(functools.partial(self.emit, 'about_btn_clicked'), strong_ref=True),
            self.view.on_settings_btn_clicked.connect(functools.partial(self.emit, 'settings_btn_clicked'), strong_ref=True),
            self.view.on_ift_btn_clicked.connect(functools.partial(self.emit, 'ift_btn_clicked'), strong_ref=True),
            self.view.on_conan_btn_clicked.connect(functools.partial(self.emit, 'conan_btn_clicked'), strong_ref=True),
        ]
...
class MainMenuWindow(_MainMenuWindow):
    def __init__(self, *args, **kwargs):
        super().__init__(
            _view_cls=MainMenuWindowView,
            _presenter_cls=MainMenuWindowPresenter,
            *args, **kwargs
        )
