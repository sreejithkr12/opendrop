...
class MainView(GtkWindowView, IMainView):
    class _Events(GtkWindowView._Events):
        def __init__(self):
            ...
            self.on_text_entered = Event()
            self.on_entry_button_clicked = Event()
    ...
    def setup(self) -> None:
        ...
        entry = Gtk.Entry()
        entry_button = Gtk.Button(label="Print Text") # used for debugging
        ...
        grid.attach(entry_button, 0, 5, 1, 1)
