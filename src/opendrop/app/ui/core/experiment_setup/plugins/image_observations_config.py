...
class PluginModel(Model):
    def __init__(self, experiment_setup_model: ExperimentSetup):
        ...
        self._num_frames = None  # type: Optional[int]
        self._frame_interval = None  # type: Optional[float]

        self.routes = [
            data_binding.Route.b_to_a(type(self.experiment_setup_model).frame_timestamps, type(self).frame_timestamps)
        ]

        data_binding.bind(self.experiment_setup_model, self, self.routes)
        data_binding.poke(self)

        self.frame_timestamps = self.experiment_setup_model.observer.timestamps
    ...
    def frame_timestamps_changed(self) -> None:
        try:
            self.experiment_setup_model.observer.timestamps = self.frame_timestamps
        except ValueError:
            pass
...
class Plugin(ExperimentSetupPlugin):
    ...
    def should_load(cls, model: ExperimentSetup) -> bool:
        if isinstance(model.observer, ImageSlideshowObserver):
            return True
    ...
    def setup(self) -> None:
        ...
        self.routes = [
            *data_binding.Route.both(type(self.plugin_model).frame_interval, type(self.dash_view).frame_interval),
            data_binding.Route.a_to_b(type(self.plugin_model).num_frames, type(self.dash_view).num_frames),
        ]
...
class DashView(observations_config.DashView):
    ...
    def setup(self):
        ...
        self.num_frames_input.props.sensitive = False
