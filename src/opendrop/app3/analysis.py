...
class IFTAnalysisConfiguration:
    def __init__(self, parent: 'IFTAnalysis'):
        ...
        self.bn_num_frames = AtomicBindableAdapter(
            getter=lambda: 303,
            setter=lambda v: print('setsting', v))
        # ) AtomicBindableVar(None)  # type: AtomicBindable[Optional[int]]
        self.bn_frame_interval = AtomicBindableVar(None)  # type: AtomicBindable[Optional[float]]

        self.bn_canny_min_thresh = AtomicBindableAdapter(
            getter=lambda: 30,
            setter=lambda v: print('setting', v))
        # ) AtomicBindableVar(30)
